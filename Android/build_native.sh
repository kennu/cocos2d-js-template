...
COCOS2DX_ROOT="$HOME/Applications/cocos2d-x-2.1.4"
APP_ROOT="$DIR/.."
...
# copy resources
for file in "$APP_ROOT"/published/Android/*
do
if [ -d "$file" ]; then
...
cp "$COCOS2DX_ROOT"/scripting/javascript/bindings/js/* "$APP_ANDROID_ROOT"/assets
...
if [[ "$buildexternalsfromsource" ]]; then
    ...
    "$NDK_ROOT"/ndk-build -C "$APP_ANDROID_ROOT" $* "NDK_MODULE_PATH=${COCOS2DX_ROOT}:${COCOS2DX_ROOT}/cocos2dx/platform/third_party/android/prebuilt"
fi
